...
void lemlib::Chassis::driveToHeading(float distance, float theta, int timeout, driveToHeadingParams params, bool async) {
    ...
    if (async) {
        pros::Task task([&]() { driveToHeading(distance, theta, timeout, params, false); });
        this->endMotion();
   ...
   // target.theta = lastPose.angle(target);
    ...
    while (!timer.isDone() && ((!lateralSmallExit.getExit() && !lateralLargeExit.getExit()) || !close) &&
        ...
        const float angularError = angleError(adjustedRobotTheta,theta);
        ...
        if (!close) lateralOut = slew(lateralOut, prevLateralOut, 0);
