...
void lemlib::Chassis::moveToPoint(float x, float y, int timeout, MoveToPointParams params, bool async) {
    ...
    // main loop
    while ((!timer.isDone() && params.minSpeed == 0 && ((!lateralSmallExit.getExit() && !lateralLargeExit.getExit()) || !close) && this->motionRunning) || (!timer.isDone() && this->motionRunning && (params.minSpeed > 0 ))) {
        // update position
        ...
        if (!close || params.minSpeed > 0) lateralOut = slew(lateralOut, prevLateralOut, 0);
