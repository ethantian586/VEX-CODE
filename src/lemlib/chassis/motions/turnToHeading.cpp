...
void lemlib::Chassis::turnToHeading(float theta, int timeout, TurnToHeadingParams params, bool async) {
    ...
    // main loop
    while ((!timer.isDone() && !angularLargeExit.getExit() && params.minSpeed == 0 && !angularSmallExit.getExit() && this->motionRunning) || (!timer.isDone() && params.minSpeed > 0 && this->motionRunning)) {
        // update variables
        Pose pose = getPose();
