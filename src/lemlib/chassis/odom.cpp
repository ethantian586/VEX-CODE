...
void lemlib::track(){
    ...
    // else, use the the substituted tracking wheels (drivetrain)
    else {
        heading -= (deltaVertical1 - deltaVertical2) /
                   (odomSensors.vertical1->getOffset() - odomSensors.vertical2->getOffset());
    }
    ...
    // calculate change in x and y (local wheel deltas)
    float deltaX = 0;
    float deltaY = 0;
    ...
    // --- NEW: Use trackNoOdomWheel-style arc math when there are NO independent horizontal wheels
    // and we have both left & right drivetrain encoder deltas available (deltaVertical1/deltaVertical2).
    // This handles curved paths more accurately by computing per-wheel arcs then averaging.
    float localX = 0.0f;
    float localY = 0.0f;
    const float EPS = 1e-6f;

    bool haveBothDriveEncoders = (odomSensors.vertical1 != nullptr && odomSensors.vertical2 != nullptr);
    bool haveHorizontalOdom = (horizontalWheel != nullptr);

    if (!haveHorizontalOdom && haveBothDriveEncoders) {
        // deltaVertical1/2 are already distances (same units as rawVertical)
        float deltaLeft  = deltaVertical1;
        float deltaRight = deltaVertical2;

        // prefer IMU for deltaHeading if we used it above, otherwise deltaHeading computed from wheels was used earlier
        // deltaHeading is already set above consistent with LemLib priority
        if (fabs(deltaHeading) < EPS) {
            localY = (deltaLeft + deltaRight) / 2.0f;
        } else {
            // track width: distance between left and right encoder contact points
            float trackWidth = fabs(odomSensors.vertical1->getOffset() - odomSensors.vertical2->getOffset());
            // sin multiplier (same as trackNoOdomWheel)
            float sin_mult = 2.0f * sinf(deltaHeading / 2.0f);

            // compute arc displacement for each wheel, including wheel separation/offset
            float delta_local_y_left  = sin_mult * (deltaLeft  / deltaHeading + trackWidth / 2.0f);
            float delta_local_y_right = sin_mult * (deltaRight / deltaHeading + trackWidth / 2.0f);

            localY = (delta_local_y_left + delta_local_y_right) / 2.0f;
        }
        localX = 0.0f; // differential drive cannot strafe (no horizontal odom)
    } else {
        // Default LemLib behavior (works with single substituted vertical or with real odom wheels)
        if (fabs(deltaHeading) < EPS) { // prevent divide by 0
            localX = deltaX;
            localY = deltaY;
        } else {
            localX = 2.0f * sinf(deltaHeading / 2.0f) * (deltaX / deltaHeading + horizontalOffset);
            localY = 2.0f * sinf(deltaHeading / 2.0f) * (deltaY / deltaHeading + verticalOffset);
        }
    }
    ...
    // calculate global x and y (rotate local by average heading)
